(ns gbfs.main
  "Kotodama WASM entrypoint for the Gbfs clean-room actor (L4) — Clojure port.
  ...
  facts (`gbfs.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "SystemInformation" :plural "systeminformations" :id-prefix "gbfs_sys"
    :fields [:systemId :name :operator :url :timezone :openingHours :phoneNumber :email :feedContactEmail]
    :required [:name :operator]
    :coerce {}
    :refs {}}
   {:entity "VehicleType" :plural "vehicletypes" :id-prefix "gbfs_veh"
    :fields [:vehicleTypeId :formFactor :propulsionType :returnConstraint :maxRangeMeters :riderCapacity :wheelCount :maxPermittedSpeed :minAge]
    :required [:formFactor :propulsionType]
    :coerce {:maxRangeMeters :float :riderCapacity :int :wheelCount :int :maxPermittedSpeed :int :minAge :int}
    :refs {}}
   {:entity "Station" :plural "stations" :id-prefix "gbfs_sta"
    :fields [:stationId :name :lat :lon :address :city :regionId :capacity :isVirtualStation :isChargingStation :parkingType]
    :required [:name :lat]
    :coerce {:lat :float :lon :float :capacity :int :isVirtualStation :bool :isChargingStation :bool}
    :refs {:stationId "Station" :regionId "Region"}}
   {:entity "StationStatus" :plural "stationstatuses" :id-prefix "gbfs_sta"
    :fields [:stationId :numVehiclesAvailable :numDocksAvailable :isInstalled :isRenting :isReturning :lastReported]
    :required [:numVehiclesAvailable :numDocksAvailable]
    :coerce {:numVehiclesAvailable :int :numDocksAvailable :int :isInstalled :bool :isRenting :bool :isReturning :bool}
    :refs {:stationId "Station"}}
   {:entity "Vehicle" :plural "vehicles" :id-prefix "gbfs_veh"
    :fields [:vehicleId :lat :lon :isReserved :isDisabled :vehicleTypeId :currentRangeMeters :lastReported]
    :required [:lat :lon]
    :coerce {:lat :float :lon :float :isReserved :bool :isDisabled :bool :currentRangeMeters :float}
    :refs {:vehicleId "Vehicle" :vehicleTypeId "VehicleType"}}
   {:entity "Region" :plural "regions" :id-prefix "gbfs_reg"
    :fields [:regionId :name]
    :required [:name]
    :coerce {}
    :refs {:regionId "Region"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"gbfs.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "gbfs-compat" :tier tier :entities entities} 200])
